// detector-config-drift reads events JSONL from stdin and emits findings
// JSONL to stdout for configuration change events: security group modifications,
// IAM policy changes, MFA disabling, and audit log modifications.
//
// Usage:
//
//	detector-config-drift --baseline <path> < events.jsonl
package main
